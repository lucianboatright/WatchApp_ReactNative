    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.approvedPosts}>
                <FlatList
                    data={approvedPost}
                    renderItem={({item}) => <Rendering post={item.data().post}  timeStamp={item.data().timeStamp} userName={userName} approved={item.data().approved} onApprove={() => onApproval(item.data().id)} onReject={() => onRegect(item.data().id)} />} 
                />
            </View>
        </View>
    )
